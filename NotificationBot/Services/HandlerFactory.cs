...
namespace NotificationsBot.Services
{
    public class HandlerFactory : IHandlerFactory
    {
        ...
        public async Task ProcessHandler(Type handlerType, string json)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка десереализации объекта с типом {Name}", payloadType.Name);
                throw;
            }
            ...
            {
                _logger.LogError(ex, "Ошибка во время выполнения обработчика {Name}", handlerType.Name);
                throw;
            }
        }
    }
